...
public class Matrix_OLD extends NumericalBase {
    protected int rowsCount;
    protected int columnsCount;
    protected double[][] matrix;
    public Matrix_OLD(String pathToFile) throws FileNotFoundException
    {
    ...
    public Matrix_OLD(int rowsCount, int columnsCount)
    {
        this.rowsCount = rowsCount;
        this.columnsCount = columnsCount;
        this.matrix = new double[rowsCount][columnsCount];
    ...
    public Matrix_OLD(double[][] matrix, int rowsCount, int columnsCount)
    {
        this.rowsCount = rowsCount;
        this.columnsCount = columnsCount;
        this.matrix = matrix;
    ...
    public Matrix_OLD cloneMatrix()
    {
        Matrix_OLD cloneMatrix = new Matrix_OLD(this.rowsCount, this.columnsCount);
        for (int i = 0; i < this.rowsCount; i++)
            for (int j = 0; j < this.columnsCount; j++)
    ...
    public static Matrix_OLD createNewMatrix()
    {
        Matrix_OLD matrix = new Matrix_OLD();
        matrix.initializeMatrix();
        return matrix;
    }
    public static Matrix_OLD createNewMatrix(int rowsCount, int columnsCount)
    {
        Matrix_OLD matrix = new Matrix_OLD(rowsCount, columnsCount);
        matrix.initializeMatrix();
        return matrix;
    }
    public static Matrix_OLD createNewRandomIntMatrix(int from, int to)
    {
        Matrix_OLD matrix = new Matrix_OLD();
        matrix.initializeRandomIntMatrix(from, to);
        return matrix;
    }
    public static Matrix_OLD createNewRandomIntMatrix(int rowsCount, int columnsCount, int from, int to)
    {
        Matrix_OLD matrix = new Matrix_OLD(rowsCount, columnsCount);
        matrix.initializeRandomIntMatrix(from, to);
        return matrix;
    }
    public static Matrix_OLD createNewRandomMatrix(double from, double to)
    {
        Matrix_OLD matrix = new Matrix_OLD();
        matrix.initializeRandomMatrix(from, to);
        return matrix;
    }
    public static Matrix_OLD createNewRandomMatrix(int rowsCount, int columnsCount, double from, double to)
    {
        Matrix_OLD matrix = new Matrix_OLD(rowsCount, columnsCount);
        matrix.initializeRandomMatrix(from, to);
        return matrix;
    ...
    public void deleteRow(int index)
    {
        ...
        {
            Matrix_OLD newMatrix = new Matrix_OLD(this.rowsCount - 1, this.columnsCount);
            int tmpIndex = 0;
            for (int i = 0; i < this.rowsCount; i++)
                if (i != index)
                {
    ...
    public void deleteColumn(int index)
    {
        ...
        {
            Matrix_OLD newMatrix = new Matrix_OLD(this.rowsCount, this.columnsCount - 1);
            int tmpIndex = 0;
    ...
    public Matrix_OLD partOfMatrix(int leftBorder, int rightBorder, int upBorder, int downBorder)
    {
        ...
        return new Matrix_OLD(matrixPart, newRowsCount, newColumnsCount);
    }
    public Matrix_OLD matrixAddition(Matrix_OLD addMatrix)
    {
        ...
        {
            Matrix_OLD resultMatrix = new Matrix_OLD(this.rowsCount, addMatrix.columnsCount);
    ...
    public Matrix_OLD constantMultiplication(double constant)
    {
        double[][] newMatrix = this.matrix;
        for (int i = 0; i < this.rowsCount; i++)
            for (int j = 0; j < this.columnsCount; j++)
            { newMatrix[i][j] *= constant; }
        return new Matrix_OLD(newMatrix, this.rowsCount, this.columnsCount);
    }
    public Matrix_OLD matrixMultiplication(Matrix_OLD addMatrix)
    {
        ...
        {
            Matrix_OLD resultMatrix = new Matrix_OLD(this.rowsCount, addMatrix.getColumnsCount());
            for (int i = 0; i < this.rowsCount; i++)
            {
    ...
    public Matrix_OLD matrixPow(int pow)
    {
        Matrix_OLD matrixPow = this.cloneMatrix();
    ...
    public Matrix_OLD transposition()
    {
        double[][] transpositionMatrix = new double[this.columnsCount][this.rowsCount];
        for(int i = 0; i < this.columnsCount; i++)
            for (int j = 0; j < this.columnsCount; j++)
            { transpositionMatrix[i][j] = this.getItem(j, i); }
        return new Matrix_OLD(transpositionMatrix, this.columnsCount, this.rowsCount);
    }

    public boolean isMatrixEqual(Matrix_OLD compareMatrix)
    {
    ...
    public Matrix_OLD gaussianTransform()
    {
        Matrix_OLD cloneMatrix = this.cloneMatrix();
    ...
    public double gaussianDeterminant()
    {
        ...
        Matrix_OLD tempMatrix = new Matrix_OLD(copyMatrix, this.rowsCount, this.columnsCount);
    ...
    public Matrix_OLD setMinor(int rowIndex, int colIndex)
    {
        Matrix_OLD minor = this.cloneMatrix();
        minor.deleteRow(rowIndex);
        minor.deleteColumn(colIndex);
        return minor.cloneMatrix();
    }
    public Matrix_OLD inversion()
    {
        Matrix_OLD inversiveMatrix = new Matrix_OLD(this.rowsCount, this.columnsCount);
        ...
        else
        {
            ...
            else
            {
                for (int i = 0; i < this.rowsCount; i++)
                {
                    for (int j = 0; j < this.columnsCount; j++)
                    {
                        Matrix_OLD minor = this.setMinor(i, j);
    ...
    public EigenPair advancedPowMethod(int pow, Vector y_0) throws ReflectiveOperationException, IOException {
        switch (InputStreamParser.parseChoiceOfTwo("Max", "Min"))
        {
            ...
            case 2:
                ...
                Matrix_OLD singleMatrix = new Matrix_OLD(this.rowsCount, this.columnsCount);
                singleMatrix.createSingleMatrix();
                singleMatrix = singleMatrix.constantMultiplication(-biggestLambda);
                Matrix_OLD newMatrix = this.matrixAddition(singleMatrix).cloneMatrix();
